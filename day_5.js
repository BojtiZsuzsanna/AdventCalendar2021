...
// function overlapCalculation(twoDInput) {
//     let overlaps = [];
//     let actualArrayCoordinates = [];

//     for (let i = 0; i < twoDInput.length; i++) {
//         let lengthOfDiffCoord = 0;

//         if (twoDInput[i][0] === twoDInput[i][2]) {
//             lengthOfDiffCoord = Math.abs(twoDInput[i][1] - twoDInput[i][3]);
//             let firstCoordinate = twoDInput[i][0];
//             for (let j = 0; j <= lengthOfDiffCoord; j++) {
//                 let coordinates = [];
//                 let secondCoordinate = twoDInput[i][1] + j;
//                 coordinates.push(firstCoordinate, secondCoordinate);

//                 implementCoordinates(actualArrayCoordinates, coordinates, overlaps)
//             }
//         } else {
//             lengthOfDiffCoord = Math.abs(twoDInput[i][0] - twoDInput[i][2]);
//             let secondCoordinate = twoDInput[i][1];
//             for (let j = 0; j <= lengthOfDiffCoord; j++) {
//                 let coordinates = [];
//                 let firstCoordinate = twoDInput[i][0] + j;
//                 coordinates.push(firstCoordinate, secondCoordinate);
...
// console.log(overlapCalculation(twoDInput));
...
function checkOverlaps(input) {
    let listOfCoordinates = [];
    let listOfOverlaps = [];
    for (let i = 0; i < input.length; i++) {
        let diffFirst = input[i][2] - input[i][0];
        let diffSec = input[i][3] - input[i][1];
        let diff = 0;

        if (Math.abs(diffFirst) > Math.abs(diffSec)) {
            diff = diffFirst;
        } else if (Math.abs(diffSec) > Math.abs(diffFirst)) {
            diff = diffSec;
        } else if (Math.abs(diffSec) === Math.abs(diffFirst)){
            diff = diffFirst;
        }

        let newFirstCoord = input[i][0];
        let newSecCoordinate = input[i][1];
        for (let j = 0; j <= Math.abs(diff); j++) {

            if ((Math.abs(diffFirst) / diffFirst)) {
                newFirstCoord = input[i][0] + (Math.abs(diffFirst) / diffFirst)*j;
            }

            if ((Math.abs(diffSec) / diffSec)) {
                newSecCoordinate = input[i][1] + (Math.abs(diffSec) / diffSec)*j;
            }
            let newCoordinate = [newFirstCoord, newSecCoordinate];

            if (listOfCoordinates.length === 0) {
                listOfCoordinates.push(newCoordinate);
                listOfOverlaps.push(1);
            } else {
                let checker = false;
                let overlapIndex = 0;
                for (let k = 0; k < listOfCoordinates.length; k++) {
                    if (listOfCoordinates[k][0] === newCoordinate[0] && listOfCoordinates[k][1] === newCoordinate[1]) {
                        checker = true;
                        overlapIndex = k;
                    }
                }
    ...
    let numberOfOverlaps = 0;
    for (let i = 0; i < listOfOverlaps.length; i++) {
        if (listOfOverlaps[i] > 1) {
            numberOfOverlaps++;
        }
    }
...
console.log(checkOverlaps(input));
